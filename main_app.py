...
if load_res:
    ...
    def search_stock(search_term):
        ...
        if is_chosung:
            return [s for s in df['종목명'].tolist() if search_term in get_chosung(s)][:15] # 상위 15개만
        return [s for s in df['종목명'].tolist() if search_term.lower() in s.lower()][:15]

    def search_theme(search_term):
        if not search_term: return []
        is_chosung = all(char in "ㄱㄲㄴㄷㄸㄹㅁㅂㅃㅅㅆㅇㅈㅉㅊㅋㅌㅍㅎ " for char in search_term)
        if is_chosung:
            return [t for t in unique_themes if search_term in get_chosung(t)][:15]
        return [t for t in unique_themes if search_term.lower() in t.lower()][:15]
